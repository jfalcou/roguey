...
void Systems::attack(Registry& reg, EntityID a_id, EntityID d_id, MessageLog& log, sol::state& lua) {
    ...
    if (a_id == reg.player_id) {
        log.add("You hit for " + std::to_string(a.damage), ColorPair::Default);
    } else {
        log.add("Monster hits you for " + std::to_string(a.damage), ColorPair::Orc);
    }
...
void Systems::check_level_up(Registry& reg, MessageLog& log, sol::state& lua) {
    ...
    if (s.xp >= next_lvl_xp) {
        ...
        if (level_res.valid()) {
            ...
            log.add("Level Up! You are now Level " + std::to_string(s.level), ColorPair::Gold);
        }
    }
}
...
void Systems::move_monsters(Registry& reg, const Dungeon& map, MessageLog& log, sol::state& lua) {
    ...
    for (auto m_id : reg.monsters) {
        ...
        const std::string& script = reg.script_paths[m_id];
