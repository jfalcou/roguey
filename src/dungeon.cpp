...
void Dungeon::generate() {
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        if (std::none_of(rooms.begin(), rooms.end(), [&](const Rect& r) { return room.intersects(r); })) {
            for (int y = room.y; y < room.y + room.h; ++y)
                for (int x = room.x; x < room.x + room.w; ++x) grid[y][x] = '.';
            if (!rooms.empty()) {
                Position p1 = rooms.back().center(), p2 = room.center();
                if (gen() % 2) { carve_h(p1.x, p2.x, p1.y); carve_v(p1.y, p2.y, p2.x); }
                else { carve_v(p1.y, p2.y, p1.x); carve_h(p1.x, p2.x, p2.y); }
            }
...
void Dungeon::carve_h(int x1, int x2, int y) { for (int x = std::min(x1, x2); x <= std::max(x1, x2); ++x) grid[y][x] = '.'; }
void Dungeon::carve_v(int y1, int y2, int x) { for (int y = std::min(y1, y2); y <= std::max(y1, y2); ++y) grid[y][x] = '.'; }
bool Dungeon::is_walkable(int x, int y) const { return (y >= 0 && y < height && x >= 0 && x < width && grid[y][x] == '.'); }
